...
func main(){
    ...
    access_token := read_token_from_file(home_dir + "/.config/cobbbot/.client")
            ...
            switch(stripped_msg[0]){
                ...
                case "settoday":
                    if(message.User.Name != "cobbcoding") {
                        ...
                        write_data_to_file(home_dir + todayfilename, strings.Join(stripped_msg[1:], " "))
                    }
                case "shoutout":
                    if(message.User.Name != "cobbcoding") {
                        client.Say(message.Channel, "You're not allowed to do that")
                ...
                case "today":
                    client.Say(message.Channel, read_token_from_file(home_dir + todayfilename))
                case "socials":
                ...
                case "69":
                    client.Say(message.Channel, "nice")
                case "nice":
                    client.Say(message.Channel, "69")
                case "discord":
                    client.Say(message.Channel, "https://discord.gg/3SkpwrRxpA")
                case "help":
                    output := ""
